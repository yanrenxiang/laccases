...
int main()
{
	...
	ann = fann_create_from_file("nn_xor_fixed.net");
#else
	printf("%s","33333333333333333333333333\n");
	ann = fann_create_from_file("nn_xor_float.net");
#endif
	...
	data = fann_read_train_from_file("nn_test01");
	for(i = 0; i < fann_length_train_data(data); i++)
	{
		fann_reset_MSE(ann);
		calc_out = fann_test(ann, data->input[i], data->output[i]);
#ifdef FIXEDFANN
		printf("CCC XOR test (%d, %d) -> %d, should be %d, difference=%f\n",
